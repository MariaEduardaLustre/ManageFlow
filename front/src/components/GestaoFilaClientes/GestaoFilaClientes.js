...
const GestaoFilaClientes = ({ onLogout }) => {
  ...
  const fetchClientesFilaCompleta = useCallback(async () => {
    ...
    try {
      const response = await api.get(
        `/empresas/fila/${idEmpresa}/${dtMovto}/${idFila}/clientes`
      );
  ...
  useEffect(() => {
    ...
    socket.on("cliente_atualizado", (data) => {
      setClientesFila((prevClientes) => {
        const clienteExistente = prevClientes.find(
          (c) => c.ID_CLIENTE === data.idCliente
        );
        if (clienteExistente) {
  ...
  const getSituacaoText = (situacao) => {
    switch (Number(situacao)) {
      case 0:
        return t("gestaoFila.status.aguardando");
      case 1:
        return t("gestaoFila.status.confirmado");
      case 2:
        return t("gestaoFila.status.naoCompareceu");
      case 3:
        return t("gestaoFila.status.chamado");
      case 4:
        return t("gestaoFila.status.atendido");
      default:
        return t("gestaoFila.status.desconhecido");
    }
  };
  ...
  const handleAdicionarCliente = async (e) => {
    ...
    if (doc.length === 11) {
      if (!isValidCPF(doc)) {
        openFeedbackModal(
          "O CPF informado é inválido. Por favor, verifique.",
          "danger"
        );
        return;
      }
      ...
      if (!isValidCNPJ(doc)) {
        openFeedbackModal(
          "O CNPJ informado é inválido. Por favor, verifique.",
          "danger"
        );
        return;
      }
      ...
      openFeedbackModal(
        "O campo CPF/CNPJ deve conter 11 ou 14 dígitos.",
        "danger"
      );
      return;
    }
    ...
    try {
      await api.post(
        `/empresas/fila/${idEmpresa}/${dtMovto}/${idFila}/adicionar-cliente`,
        novoCliente
      );
      handleCloseAddModal();
      ...
      const msg =
        err.response?.data?.error || t("gestaoFila.erros.adicionarCliente");
  ...
  const toggleBlockFila = async () => {
    ...
    try {
      ...
      openFeedbackModal(
        desired
          ? t("gestaoFila.feedback.filaBloqueada")
          : t("gestaoFila.feedback.filaDesbloqueada"),
        "success"
      );
    } catch (err) {
      const msg =
        ...
        t("gestaoFila.erros.alterarBloqueio");
      openFeedbackModal(msg, "danger");
    } finally {
      setStatusLoading(false);
    }
    ...
    switch (abaAtiva) {
      case "aguardando":
        return clientesFila.filter(
          (c) => Number(c.SITUACAO) === 0 || Number(c.SITUACAO) === 3
        );
      case "confirmados":
        return clientesFila.filter(
          (c) => Number(c.SITUACAO) === 1 || Number(c.SITUACAO) === 4
        );
      case "nao-compareceu":
        return clientesFila.filter((c) => Number(c.SITUACAO) === 2);
      default:
        return [];
    }
  };
  ...
  return (
    <div className="home-container">
      ...
      <main className="gestao-fila-page main-content">
        {/* HEADER: Voltar + Título + Bloquear/Desbloquear */}
        <section className="gestao-header-card">
          <div className="gestao-header-row">
            <div className="gestao-header-left">
              <h2 className="gestao-header-title">{t("gestaoFila.titulo")}</h2>
            </div>
            <div className="gestao-header-right">
              <Button
              ...
              <Button
                ...
                title={
                  isBlocked
                    ? t("gestaoFila.botoes.desbloquearTitle")
                    : t("gestaoFila.botoes.bloquearTitle")
                }
              >
                {statusLoading ? (
                  <>
        ...
        {/* SEÇÃO LISTA: botões (à direita) + abas + tabela */}
        <section className="clientes-fila-section">
          <div className="lista-toolbar">
            <div className="lista-toolbar-right">
              <Button
                variant="primary"
          ...
          {isBlocked && (
            <Alert variant="warning" className="mb-3">
              <strong>{t("gestaoFila.alertaBloqueio.titulo")}</strong>{" "}
              {t("gestaoFila.alertaBloqueio.texto")}
            </Alert>
          )}
          ...
          <div className="tabs-container">
            <div className="tabs-list">
              ...
              <button
                className={`tab-button ${
                  abaAtiva === "nao-compareceu" ? "active" : ""
                }`}
                ...
                <tbody>
                  {clientesFiltrados.map((cliente) => (
                    ...
                    >
                      ...
                      <td>
                        {formatarHora(cliente.DT_ENTRA)} -{" "}
                        {formatarData(cliente.DT_MOVTO)}
                      </td>
                      <td className="coluna-status-acoes">
                        <div className="conteudo-status-acoes">
                          <div className="situacao-wrapper">
                            <span
                              className={`situacao-badge ${getSituacaoClass(
                                cliente.SITUACAO
                              )}`}
                            >
      ...
      <Modal show={showAddModal} onHide={handleCloseAddModal} centered>
        ...
        <Form onSubmit={handleAdicionarCliente}>
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              <Form.Label>
                {t("gestaoFila.modalAdicionar.nascimento")}
              </Form.Label>
              <Form.Control
                type="date"
                name="DT_NASC"
            ...
            <Form.Group className="mb-3">
              <Form.Label>
                {t("gestaoFila.modalAdicionar.notificacao")}
              </Form.Label>
              <Form.Select
            ...
            <div className="d-flex gap-3">
              ...
              <Form.Group className="flex-grow-1">
                <Form.Label>
                  {t("gestaoFila.modalAdicionar.celular")}
                </Form.Label>
                <Form.Control
                  type="text"
                  name="NR_CEL"
          ...
          <Modal.Footer>
            ...
            <Button variant="primary" type="submit">
              {t("gestaoFila.modalAdicionar.adicionar")}
            </Button>
          </Modal.Footer>
        </Form>
      </Modal>
