...
/* ---------------------------------------------------------------------
	LISTA FILAS + CONFIG (Back-end sem filtros)
--------------------------------------------------------------------- */
exports.listarFilasComConfiguracao = async (req, res) => {
	...
	// Remoção de todas as flags de filtro para garantir que o SQL traga todas as filas da empresa.
...
exports.countByFila = async (req, res) => {
	...
	try {
		...
		// CORREÇÃO: Removendo caracteres invisíveis/espaços em branco na template string SQL
...
exports.toggleBlockByConf = async (req, res) => {
	...
	try {
		...
		const situacaoVal = blocked ? 0 : 1; // ✅ SITUACAO = 0 (Inativa) quando Bloqueada = 1
...
exports.applyConfigToToday = async (req, res) => {
	...
	// CORREÇÃO: Sintaxe do 'if' e tratamento de idConfFila
	if (idConfFila === undefined || idConfFila === null) { 
