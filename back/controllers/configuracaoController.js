...
/**
 * Normaliza a entrada de imagem vinda do front (chave, url ou dataURL),
...
/**
 * Garante que a configuração do dia (tabela fila) reflita a situação efetiva da ConfiguracaoFila.
 */
...
exports.getConfiguracao = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...
      nome: c.NOME_FILA,
      img_logo: logoJson
        ? { key: logoJson.key, url: await makeImageAccessUrl(logoJson.key) }
        : null,
  ...
  try {
    ...
    const [result] = await conn.execute(
      `INSERT INTO ConfiguracaoFila
         (ID_EMPRESA, NOME_FILA, TOKEN_FILA, INI_VIG, FIM_VIG, CAMPOS, MENSAGEM,
          IMG_BANNER, IMG_LOGO, TEMP_TOL, QDTE_MIN, QTDE_MAX, PER_SAIR, PER_LOC, SITUACAO)
       VALUES
         (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
  ...
  try {
    ...
    const [updResult] = await conn.execute(
      `UPDATE ConfiguracaoFila
          SET ID_EMPRESA = ?, NOME_FILA = ?, INI_VIG = ?, FIM_VIG = ?, CAMPOS = ?, MENSAGEM = ?,
              IMG_BANNER = ?, IMG_LOGO = ?, TEMP_TOL = ?, QDTE_MIN = ?, QTDE_MAX = ?,
...
exports.listarFilasPorEmpresa = async (req, res) => {
  ...
  const sql = `
    SELECT cf.ID_CONF_FILA, cf.NOME_FILA,
           ...
           cf.DT_CRIACAO AS data_configuracao, cf.DT_ALTERACAO AS data_atualizacao
      FROM configuracaofila cf
...
exports.getPublicStatusByToken = async (req, res) => {
  ...
  try {
    const [[cfg]] = await db.query(
      `SELECT cf.ID_CONF_FILA, cf.ID_EMPRESA, cf.NOME_FILA, cf.SITUACAO, cf.PER_SAIR,
              cf.INI_VIG, cf.FIM_VIG,
...
exports.publicLeaveByToken = async (req, res, io) => {
  ...
  try {
    ...
    const [[cfg]] = await conn.query(
      `SELECT cf.ID_EMPRESA, cf.ID_CONF_FILA, cf.PER_SAIR, f.ID_FILA
         FROM ConfiguracaoFila cf
    LEFT JOIN fila f
           ON f.ID_EMPRESA = cf.ID_EMPRESA
          AND f.ID_CONF_FILA = cf.ID_CONF_FILA
